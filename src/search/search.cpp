...
float negamax(int depth, Board& board, float alpha, float beta, int colour) {
  ...
  if (depth == 0) {
    ...
    auto captures = std::remove_if(
        captureSet.begin(), captureSet.end(), [](const Move& move) {
          return (move.getFlag() == Flag::CAPTURE ||
...
float quiescence_search(Board& board, int depth, int colour, float alpha,
                        float beta) {
  ...
  for (size_t i = 0; i < captures.size(); i++) {
    ...
    board.makeMove(move);
    value = std::max(
        value, -quiescence_search(board, depth - 1, -colour, -beta, -alpha));
    board.unmakeMove();
