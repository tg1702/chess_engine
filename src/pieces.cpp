...
PieceSet::PieceSet(const PieceBB& w_args, const PieceBB& b_args) {
  Bitboard w_all_pieces = w_args.king_bb | w_args.knight_bb | w_args.pawn_bb |
                          w_args.rook_bb | w_args.bishop_bb | b_args.queen_bb;

  Bitboard b_all_pieces = b_args.king_bb | b_args.knight_bb | b_args.pawn_bb |
                          b_args.rook_bb | b_args.bishop_bb | b_args.queen_bb;
...
void PieceSet::setBoard(const PieceBB& w_args, const PieceBB& b_args) {
  Bitboard w_all_pieces = w_args.king_bb | w_args.knight_bb | w_args.pawn_bb |
                          w_args.rook_bb | w_args.bishop_bb | w_args.queen_bb;
...
bool PieceSet::canKingSideCastle(Side side) {
  if (side == Side::WHITE)
    return (Pieces[side][KING] & utils::to_bb(E1)) &&
           ...
           ((1ULL << F1) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           ((1ULL << G1) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           !isAttacked(side, E1) && !isAttacked(side, F1) &&
           !isAttacked(side, G1);
  else
    return (Pieces[side][KING] & utils::to_bb(E8)) &&
           (Pieces[side][ROOK] & utils::to_bb(H8)) &&
           ((1ULL << F8) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           ((1ULL << G8) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           !isAttacked(side, E8) && !isAttacked(side, F8) &&
           !isAttacked(side, G8);
}
...
bool PieceSet::isAttacked(Side side, Square square) {
  ...
  int bishopIndex = utils::generateMagicIndex(
      (this->Pieces[side][ALL] | this->Pieces[opponent][ALL]) &
          bishopOccupancyMasks[square],
      bishopMagics[square], square, 1);

  int rookIndex = utils::generateMagicIndex(
      (this->Pieces[side][ALL] | this->Pieces[opponent][ALL]) &
          rookOccupancyMasks[square],
      rookMagics[square], square, 0);

  if (this->Pieces[opponent][PAWN] & pawnAttackLookups[side][square])
    return true;
  if (this->Pieces[opponent][KNIGHT] & knightLookups[square]) return true;
  if (this->Pieces[opponent][KING] & kingLookups[square]) return true;
  ...
  if (this->Pieces[opponent][BISHOP] & bishopMoveList[square][bishopIndex])
    return true;
  if ((bishopMoveList[square][bishopIndex] | rookMoveList[square][rookIndex]) &
      this->Pieces[opponent][QUEEN])
    return true;
...
bool PieceSet::canQueenSideCastle(Side side) {
  if (side == Side::WHITE)
    return (Pieces[side][KING] & utils::to_bb(E1)) &&
           ...
           ((1ULL << D1) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           !isAttacked(side, C1) && !isAttacked(side, D1) &&
           !isAttacked(side, E1);
  else
    return (Pieces[side][KING] & utils::to_bb(E8)) &&
           ...
           ((1ULL << D8) &
            ~(Pieces[Side::WHITE][ALL] | Pieces[Side::BLACK][ALL])) &&
           !isAttacked(side, C8) && !isAttacked(side, D8) &&
           !isAttacked(side, E8);
}
