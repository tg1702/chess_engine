...
int main() {
  ...
  while (std::getline(std::cin, s)) {
    ...
    if (words[0] == "position" && words[1] == "fen") {
      ...
      std::string fen = words[2] + " " + words[3] + " " + words[4] + " " +
    ...
    if (words[0] == "go") {
      ...
      int defaultTime =
          100000;  // TODO: add movetime argument and reset default to INT_MAX
      ...
      if (wtime != -1 && btime != -1 && words.size() > 1) {
        ...
        (turn == -1) ? defaultTime = btimeVal / 25
                     : defaultTime = wtimeVal / 25;
      ...
      if (winc != -1 && binc != -1 && words.size() > 1) {
        ...
        (turn == -1) ? defaultTime += wincVal / 25
                     : defaultTime += bincVal / 25;
      }
      ...
      if (words[1] == "perft" && (words.size() == 3)) {
        ...
        // board.setFEN("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0
        // 1");
        t.start();
        divide(DEPTH, board);
        t.stop();
        ...
        //}
        // if (!s_thread.joinable()){
        // s_thread = std::thread(search, std::ref(board), turn, defaultTime,
        // depth);
