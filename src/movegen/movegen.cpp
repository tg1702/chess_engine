...
void MoveGen::setState(BoardState& state) { this->state = state; }
...
void MoveGen::generateCaptures(MoveList* move_list) {
  ...
  (state.turn == Side::WHITE)
      ? generateWhitePawnMoves<MoveType::CAPTURES>(move_list)
      : generateBlackPawnMoves<MoveType::CAPTURES>(move_list);
...
void MoveGen::generateQuiets(MoveList* move_list) {
  ...
  (state.turn == Side::WHITE)
      ? generateWhitePawnMoves<MoveType::QUIETS>(move_list)
      : generateBlackPawnMoves<MoveType::QUIETS>(move_list);
...
void MoveGen::addPossibleMove(Square start, Bitboard positions, PieceType type,
                              MoveList* move_list) {
  while (positions != 0) {
...
void MoveGen::generateKingMoves(MoveList* move_list) {
  ...
  while (king_bb != 0ULL) {
    ...
    if (Type == MoveType::CAPTURES) {
      addPossibleMove<MoveType::CAPTURES>(from, legalMove & state.enemies, KING,
                                          move_list);
    }

    else {
      addPossibleMove<MoveType::QUIETS>(from, legalMove & ~state.enemies, KING,
                                        move_list);
    }
  }
  ...
  if (Type == MoveType::QUIETS) {
    if (state.whiteKSCastle) {
      move_list->moves[move_list->count] =
          Move(Flag::W_KS_CASTLE, E1, G1, KING, KING);
      move_list->count++;
    }

    if (state.whiteQSCastle) {
      move_list->moves[move_list->count] =
          Move(Flag::W_QS_CASTLE, E1, C1, KING, KING);
      move_list->count++;
    }

    if (state.blackKSCastle) {
      move_list->moves[move_list->count] =
          Move(Flag::B_KS_CASTLE, E8, G8, KING, KING);
      move_list->count++;
    }
    if (state.blackQSCastle) {
      move_list->moves[move_list->count] =
          Move(Flag::B_QS_CASTLE, E8, C8, KING, KING);
      move_list->count++;
    }
  }
}
...
void MoveGen::generateKnightMoves(MoveList* move_list) {
  ...
  while (knight_bb != 0ULL) {
    ...
    if (Type == MoveType::CAPTURES) {
      addPossibleMove<MoveType::CAPTURES>(from, legalMove & state.enemies,
                                          KNIGHT, move_list);
    }

    else {
      addPossibleMove<MoveType::QUIETS>(from, legalMove & ~state.enemies,
                                        KNIGHT, move_list);
    }
  }
}
...
void MoveGen::generateWhitePawnMoves(MoveList* move_list) {
  ...
  Bitboard empty = ~all;
  Bitboard ep_squares = (state.enPassant == -1)
                            ? 0
                            : (state.pieces.pawn_bb & RANK_5 &
                               (utils::to_bb((state.enPassant - 1 - 8)) |
                                utils::to_bb((state.enPassant + 1 - 8))));
  ...
  if (Type == MoveType::QUIETS) {
    while (normal_moves != 0ULL) {
      ...
      while (legalMove != 0ULL) {
        ...
        move_list->moves[move_list->count] =
            Move(Flag::NORMAL, from, to, PAWN, KING);
        move_list->count++;
      }
    }
    ...
    while (promotions != 0ULL) {
      ...
      while (normal_promotions != 0ULL) {
        ...
        for (auto& normal_promoted : normal_promoted_codes) {
          move_list->moves[move_list->count] =
              Move(normal_promoted, from, to, PAWN, KING);
          move_list->count++;
        }
      }
    }
    ...
    while (normal_moves != 0ULL) {
      Square from = utils::pop_lsb(normal_moves);
      Bitboard pawn_attack =
          (pawnAttackLookups[Side::WHITE][from] & state.enemies);
    ...
    while (ep_squares != 0ULL) {
      ...
      move_list->moves[move_list->count] =
          Move(Flag::EN_PASSANT, from, ep_square, PAWN, PAWN);
      move_list->count++;
    }
    ...
    while (promotions != 0ULL) {
      ...
      while (captures != 0ULL) {
        ...
        for (auto& capture_promoted : capture_promoted_codes) {
          ...
          move_list->moves[move_list->count] =
              Move(capture_promoted, from, to, PAWN, capturedPieceType);
          move_list->count++;
        }
      }
    }
...
void MoveGen::generateBlackPawnMoves(MoveList* move_list) {
  ...
  if (Type == MoveType::QUIETS) {
    ...
    while (normal_moves != 0ULL) {
      Square from = utils::pop_lsb(normal_moves);
      Bitboard legalMove =
          (pawnAttackLookups[Side::BLACK][from] & state.enemies);
...
void MoveGen::generateRookMoves(MoveList* move_list) {
  ...
  while (rook_bb != 0ULL) {
    ...
    int index = utils::generateMagicIndex(all & rookOccupancyMasks[from],
                                          rookMagics[from], from, 0);

    Bitboard legalMove = rookMoveList[from][index] & ~(state.pieces.all);

    if (Type == MoveType::CAPTURES) {
      addPossibleMove<MoveType::CAPTURES>(from, legalMove & state.enemies, ROOK,
                                          move_list);
    }

    else {
      addPossibleMove<MoveType::QUIETS>(from, legalMove & ~state.enemies, ROOK,
                                        move_list);
    }
  }
}
...
void MoveGen::generateBishopMoves(MoveList* move_list) {
  ...
  while (bishop_bb != 0ULL) {
    ...
    int index = utils::generateMagicIndex((all)&bishopOccupancyMasks[from],
                                          bishopMagics[from], from, 1);
...
void MoveGen::generateQueenMoves(MoveList* move_list) {
  ...
  while (queen_bb != 0ULL) {
    ...
    int rookIndex = utils::generateMagicIndex((all)&rookOccupancyMasks[from],
                                              rookMagics[from], from, 0);

    int bishopIndex = utils::generateMagicIndex(
        (all)&bishopOccupancyMasks[from], bishopMagics[from], from, 1);

    Bitboard legalBishopMoves =
        bishopMoveList[from][bishopIndex] & ~(state.pieces.all);
    Bitboard legalRookMoves =
        rookMoveList[from][rookIndex] & ~(state.pieces.all);

    Bitboard legalQueenMoves = legalRookMoves | legalBishopMoves;

    if (Type == MoveType::CAPTURES) {
      addPossibleMove<MoveType::CAPTURES>(from, legalQueenMoves & state.enemies,
                                          QUEEN, move_list);
    }

    else {
      addPossibleMove<MoveType::QUIETS>(from, legalQueenMoves & ~state.enemies,
                                        QUEEN, move_list);
    }
  }
}