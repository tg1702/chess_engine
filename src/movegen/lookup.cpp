...
inline std::array<uint64_t, TABLE_SIZE> createBlockedBoards(uint64_t blocker_pieces, int MAX_PATTERNS){
...
inline bool fillLookupTable(std::array<uint64_t, TABLE_SIZE>& lookupTable, int MAX_PATTERNS, uint64_t attackCombos, std::array<uint64_t,TABLE_SIZE> occupancyCombos, int square,  int type, uint64_t magicNumber){
