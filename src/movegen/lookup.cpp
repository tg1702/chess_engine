...
uint64_t generateWhiteBishopMask(Square plain_square) {
  ...
  for (int i = 7; i <= 49; i += 7) {
    ...
    if (((square << i) & RANK_1) || ((square << i) & RANK_8) ||
        ((square << i) & FILE_A) || ((square << i) & FILE_H))
      break;
  }
  ...
  return upLeftDirection | downLeftDirection | upRightDirection |
         downRightDirection;
}
...
inline uint64_t calcLegalBishopMoves(Square plain_square, Bitboard occupancy) {
  ...
  // Up left
  for (rank = curRank + 1, file = curFile - 1; rank <= 7 && file >= 0;
       rank++, file--) {
  ...
  // Up right
  for (rank = curRank + 1, file = curFile + 1; rank <= 7 && file <= 7;
       rank++, file++) {
  ...
  for (rank = curRank - 1, file = curFile + 1; rank >= 0 && file <= 7;
       rank--, file++) {
  ...
  for (rank = curRank - 1, file = curFile - 1; rank >= 0 && file >= 0;
       rank--, file--) {
...
inline std::array<uint64_t, TABLE_SIZE> createBlockedBoards(
    uint64_t blocker_pieces, int MAX_PATTERNS) {
...
inline bool fillLookupTable(std::array<uint64_t, TABLE_SIZE>& lookupTable,
                            int MAX_PATTERNS, uint64_t attackCombos,
                            std::array<uint64_t, TABLE_SIZE> occupancyCombos,
                            int square, int type, uint64_t magicNumber) {
  int i = 0;
  for (i = 0; i < MAX_PATTERNS; i++) {
    int index = utils::generateMagicIndex(occupancyCombos[i], magicNumber,
                                          square, ROOK_MOVES);
...
uint64_t generateMagicNumber(int square, uint64_t blockers, int type) {
  ...
  std::array<uint64_t, TABLE_SIZE> occupancyCombos =
      createBlockedBoards(blockers, MAX_PATTERNS);
  ...
  while (true) {
    ...
    if (__builtin_popcountll((magicNumber * attackCombos) &
